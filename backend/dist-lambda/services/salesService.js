...
exports.createOutletSale = createOutletSale;
exports.getOutletSaleByOutletAndDate = getOutletSaleByOutletAndDate;
exports.getAllOutletSales = getAllOutletSales;
exports.updateOutletSale = updateOutletSale;
exports.updateBankTransfer = updateBankTransfer;
exports.deleteOutletSale = deleteOutletSale;
exports.getOutletSalesStats = getOutletSalesStats;
exports.getOutletOptions = getOutletOptions;
const dynamodb_1 = require("./dynamodb");
const TABLE = dynamodb_1.TABLES.SALES;
/**
...
function calcTotal(data, existing) {
...
async function createOutletSale(data) {
    const existing = await getOutletSaleByOutletAndDate(data.outlet, data.date);
    if (existing)
        throw new Error(`Sale for outlet ${data.outlet} on ${data.date} already exists`);
...
async function getOutletSaleByOutletAndDate(outlet, date) {
    const item = await (0, dynamodb_1.dbGet)(TABLE, { PK: `SALE#${outlet}`, SK: `DATE#${date}` });
    return item ? toSale(item) : null;
}
async function getAllOutletSales(filters) {
    if (filters?.outlet) {
...
async function updateOutletSale(outlet, date, updates) {
    ...
    const updated = await (0, dynamodb_1.dbUpdate)(TABLE, { PK: `SALE#${outlet}`, SK: `DATE#${date}` }, payload);
    return updated ? toSale(updated) : null;
}
async function updateBankTransfer(outlet, date, bankData) {
    const existing = await getOutletSaleByOutletAndDate(outlet, date);
    if (!existing)
        return null;
    // Merge bank values
    ...
    const pct = (bank, sales) => (sales > 0 ? (bank / sales) * 100 : 0);
    ...
    if (bankData.notes !== undefined)
        payload.notes = bankData.notes;
    if (bankData.verifiedBy !== undefined)
        payload.verifiedBy = bankData.verifiedBy;
    if (bankData.verifiedBy !== undefined)
        payload.verifiedAt = new Date().toISOString();
    const updated = await (0, dynamodb_1.dbUpdate)(TABLE, { PK: `SALE#${outlet}`, SK: `DATE#${date}` }, payload);
    return updated ? toSale(updated) : null;
}
async function deleteOutletSale(outlet, date) {
    await (0, dynamodb_1.dbDelete)(TABLE, { PK: `SALE#${outlet}`, SK: `DATE#${date}` });
}
async function getOutletSalesStats(filters) {
...
async function getOutletOptions() {
    const items = await (0, dynamodb_1.dbScan)(TABLE);
    const codes = new Set();
    for (const item of items) {
        if (item.outlet)
            codes.add(item.outlet);
    }
