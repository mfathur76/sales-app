@echo off
echo 🚀 Deploying Admin Report Table Header Update...
...
ssh -i %SSH_KEY% root@%SERVER_IP% "cd %FRONTEND_DIR% && npm run build && pm2 restart sales-frontend && pm2 status && echo ✅ Admin Report table header update deployed successfully!"
